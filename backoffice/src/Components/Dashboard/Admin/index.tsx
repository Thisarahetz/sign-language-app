...
import { setCurrentUserType, setIsActive } from "@redux/DrawerSlice";
...
function AdminDashBoard() {
  ...
  const modules = useAppSelector((state) => state.auth.data.user.permissions);

  function FeatureInformationsActive() {
    const validConfigurations = [
      DASHBOARD_CONTENTS_KEYS.MANAGEMENT_COCLIENT_KEY,
    ];
  ...
  const DASHBOARD_LAST_SECTION = [
    {
      ...
      dropdown: [
        ...
        {
          title: DASHBOARD_CONTENTS.MY_ACCOUNT,
          icon: "/images/MyAccount.svg",
          key: DASHBOARD_CONTENTS_KEYS.MY_ACCOUNT_KEY,
          route: APP_ROUTES.BLOGS,
        },
      ],
    },
  ];
      ...
      sidebar={
        <SideBar
          sideBarSectionFirst={
            <>
              {modules &&
                modules.map(
                  (module, index) =>
                    String(module.module_type) === "1" && (
                      <div className="sidebar_dropdown" key={index}>
                        <div
                          data-hover="false"
                          data-delay="0"
                        ...
                        >
                          ...
                          <nav className="sidebar-dropdown-list-wrapper w-dropdown-list">
                            <div className="sidebar-dropdown-inner-wrapper">
                              {module.value.map((value, index) => (
                                <a
                                  key={index}
                                  className={`sidebar-dropdown-link w-dropdown-link ${
                                    activeItem === Number(value.module_id)
                                      ? "is_active"
                                      : ""
                                  }`}
                                  onClick={() => {
                                    dispatch(
                                      setIsActive(Number(value.module_id))
                                    );
                                    navigate(value.module_route);
                                  }}
                                >
                                  {value.module_name}
                                </a>
              ...
              {modules &&
                modules.map(
                  (module) =>
                    String(module.module_type) === "0" &&
                    module.value.map((value, index) => (
                      <a
                        key={index}
                        className={`sidebar_link w-inline-block ${
                          activeItem === Number(value.module_id) ||
                          FeatureInformationsActive()
                            ? "is_active"
                            : ""
                        }`}
                        onClick={() => {
                          dispatch(setIsActive(Number(value.module_id)));
                          navigate(value.module_route);
                        }}
                      >
                        <div className="menu_icon w-embed">
          ...
          sideBarSectionSecond={
            ...
            //     return (
            //       <div className="sidebar_dropdown" key={`index-key${index}`}>
            //         <div
            //           data-hover="false"
            //           data-delay="0"
            //           className="dropdown-wrapper sidebar-dropdown w-dropdown"
