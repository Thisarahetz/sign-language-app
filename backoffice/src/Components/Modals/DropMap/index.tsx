...
export default function LocationDropDownModel({
  open,
  onClose,
}: LocationPickupModelProps) {
  const { drop_location, local_dropOff_address } = useAppSelector(
    (state) => state.order
  );
  ...
  const handleLocationUpdate = (coords: coordinates) => {
    dispatch(setDropOffLocation(coords));
  };

  const handleAddressUpdate = (address: google.maps.GeocoderResult) => {
    dispatch(setLocalDropOffAddress(address));
  };
  ...
  useEffect(() => {
    dispatch(
      setDropOffAddress(
        local_dropOff_address
          ? getLongNamesAsString(local_dropOff_address?.address_components)
          : ""
      )
    );
        ...
        <div className="w-layout-grid grid-2-column is-modal">
          ...
          >
            <div className="form-inner-wrapper">
              <h5 className="text_24 weight_700">Confirm drop off location</h5>
                  ...
                  address={
                    local_dropOff_address
                      ? getLongNamesAsString(
                          local_dropOff_address?.address_components
                        )
                      : undefined
                  }
                />
