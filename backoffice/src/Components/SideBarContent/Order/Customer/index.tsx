...
import { setCustomerID } from "@redux/OrderSlice";
...
function OrderCustomerSideBarContent() {
  const dispatch = useAppDispatch();
  const navigate = useNavigate();
  const [search_key, setSearchKey] = useState("");
  const { id } = useParams();
  const { data, isLoading, isError, refetch } = useQuery({
    queryKey: ["order-customer-side-bar-content"],
    queryFn: () => GetAllCustomersCall(1, 10, search_key),
  });

  const { customer } = useAppSelector((state) => state.order);
              ...
              <div className="sidebar_dropdown">
                ...
                >
                  <NavLink
                    ...
                    to={`/order/customers` + `/${item.id}`}
                    className={({ isActive }) =>
                      `dropdown-toggle-2 sidebar-dropdown-link w-dropdown-link ${
                        customer?.id === item.id ? "is_active" : ""
                      }`
                    }
                    end
                  >
...
export default OrderCustomerSideBarContent;
