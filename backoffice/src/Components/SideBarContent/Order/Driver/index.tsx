...
import { GetAllCustomersCall } from "@src/Api/Services/Customer";
import { GetAllDriversCall } from "@src/Api/Services/Drivers";
import { useQuery } from "@tanstack/react-query";
import { useEffect, useState } from "react";
import { NavLink, useNavigate, useParams } from "react-router-dom";

function OrderDriverSideBarContent() {
  const dispatch = useAppDispatch();
  const navigate = useNavigate();
  const [search_key, setSearchKey] = useState("");
  const { id } = useParams();
  const { data, isLoading, isError, refetch } = useQuery({
    queryKey: ["order-driver-side-bar-content"],
    queryFn: () => GetAllDriversCall(1, 10, search_key),
  });

  const [selectedUserId, setSelecedUserId] = useState(0);
            ...
            return (
              <div className="sidebar_dropdown">
                ...
                >
                  <NavLink
                    onClick={() => {
                      setSelecedUserId(item.id);
                    }}
                    to={`/order/driver` + `/${item.id}`}
                    className={({ isActive }) =>
                      `dropdown-toggle-2 sidebar-dropdown-link w-dropdown-link ${
                        selectedUserId === item.id ? "is_active" : ""
                      }`
                    }
                    end
                  >
...
export default OrderDriverSideBarContent;
